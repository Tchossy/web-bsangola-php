...
searchRegisterForm.addEventListener('submit', async event => {
  ...
  const dataFetch = await fetch(
    baseURL +
      'serviceControllers.php?typeForm=get_all_service_search&searchRegisterValue=' +
...
const listCustomer = async limitRegister => {
  const dataUsers = await fetch(
    baseURL +
      'serviceControllers.php?typeForm=get_services&numRegister=' +
      limitRegister
  )
...
cardForm.addEventListener('submit', async event => {
  ...
  const dataNewUser = await fetch(
    baseURL + 'serviceControllers.php?typeForm=create_service',
    {
      method: 'POST',
      body: dataForm
    }
...
async function confirmDelete(idCustomer) {
  await fetch(
    baseURL +
      'serviceControllers.php?typeForm=delete_services&idCustomer=' +
      idCustomer
  )
  listCustomer(numRegister.value)
}
...
async function editeCustomer(idCustomer) {
  const dataUsers = await fetch(
    baseURL +
      'serviceControllers.php?typeForm=get_service&idCustomer=' +
      idCustomer
  )
  ...
  if (response['error']) {
    ...
    const serviceData = response['dados']

    openModalEdit()

    const image_service_array = JSON.parse(serviceData.image_service)

    document.getElementById('id_edit').value = serviceData.id
    document.getElementById('images_files_edit').src = image_service_array
    document.getElementById('title_service_edit').value =
      serviceData.title_service
    document.getElementById('description_service_edit').value =
      serviceData.description_service
    document.getElementById('occupation_service_edit').value =
      serviceData.occupation_service
  }
}
...
cardEditForm.addEventListener('submit', async event => {
  ...
  const dataNewUser = await fetch(
    baseURL + 'serviceControllers.php?typeForm=edite_service',
    {
      method: 'POST',
      body: dataEditForm
    }
