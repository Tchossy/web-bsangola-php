...
searchRegisterForm.addEventListener('submit', async event => {
  ...
  const dataFetch = await fetch(
    baseURL +
      'blogsControllers.php?typeForm=get_all_blog_search&searchRegisterValue=' +
...
const listCustomer = async limitRegister => {
  const dataUsers = await fetch(
    baseURL +
      'blogsControllers.php?typeForm=get_blogs&numRegister=' +
      limitRegister
  )
...
cardForm.addEventListener('submit', async event => {
  ...
  const dataNewUser = await fetch(
    baseURL + 'blogsControllers.php?typeForm=create_blog',
    {
      method: 'POST',
      body: dataForm
    }
...
async function confirmDelete(idCustomer) {
  await fetch(
    baseURL +
      'blogsControllers.php?typeForm=delete_blogs&idCustomer=' +
      idCustomer
  )
  listCustomer(numRegister.value)
}
...
async function editeCustomer(idCustomer) {
  const dataUsers = await fetch(
    baseURL + 'blogsControllers.php?typeForm=get_blog&idCustomer=' + idCustomer
  )
  ...
  if (response['error']) {
    ...
    const blogData = response['dados']

    openModalEdit()

    const image_blog_array = JSON.parse(blogData.image_blog)

    document.getElementById('id_edit').value = blogData.id
    document.getElementById('images_files_edit').src = image_blog_array
    document.getElementById('title_blog_edit').value = blogData.title_blog
    document.getElementById('description_blog_edit').value =
      blogData.description_blog
    document.getElementById('category_blog_edit').value = blogData.category_blog
    document.getElementById('author_blog_edit').value = blogData.author_blog
    document.getElementById('epigraph_blog_edit').value = blogData.epigraph_blog
  }
}
...
cardEditForm.addEventListener('submit', async event => {
  ...
  const dataNewUser = await fetch(
    baseURL + 'blogsControllers.php?typeForm=edite_blog',
    {
      method: 'POST',
      body: dataEditForm
    }
