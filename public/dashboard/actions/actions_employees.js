...
searchRegisterForm.addEventListener('submit', async event => {
  ...
  const dataFetch = await fetch(
    baseURL +
      'employeesControllers.php?typeForm=get_all_employee_search&searchRegisterValue=' +
...
const listCustomer = async limitRegister => {
  const dataUsers = await fetch(
    baseURL +
      'employeesControllers.php?typeForm=get_employees&numRegister=' +
      limitRegister
  )
...
cardForm.addEventListener('submit', async event => {
  ...
  const dataNewUser = await fetch(
    baseURL + 'employeesControllers.php?typeForm=create_employee',
    {
      method: 'POST',
      body: dataForm
    }
...
async function confirmDelete(idCustomer) {
  await fetch(
    baseURL +
      'employeesControllers.php?typeForm=delete_employees&idCustomer=' +
      idCustomer
  )
  listCustomer(numRegister.value)
}
...
async function editeCustomer(idCustomer) {
  const dataUsers = await fetch(
    baseURL +
      'employeesControllers.php?typeForm=get_employee&idCustomer=' +
      idCustomer
  )
  ...
  if (response['error']) {
    ...
    const employeeData = response['dados']

    openModalEdit()

    const image_employee_array = JSON.parse(employeeData.image_employee)

    document.getElementById('id_edit').value = employeeData.id
    document.getElementById('images_files_edit').src = image_employee_array
    document.getElementById('name_employee_edit').value =
      employeeData.name_employee
    document.getElementById('email_employee_edit').value =
      employeeData.email_employee
    document.getElementById('occupation_employee_edit').value =
      employeeData.occupation_employee
  }
}
...
cardEditForm.addEventListener('submit', async event => {
  ...
  const dataNewUser = await fetch(
    baseURL + 'employeesControllers.php?typeForm=edite_employee',
    {
      method: 'POST',
      body: dataEditForm
    }
